...
For frequently asked questions about PSIS-LOO-CV (and related methods), including practical guidance on interpretation and diagnostics, see the [Stan LOO FAQ](https://mc-stan.org/loo/articles/online-only/faq.html).

## PSIS-LOO-CV in Arviz

Since the name and even acronym are too long and not that easy to pronounce, in ArviZ, we usually refer to this and related methods as LOO. For instance, to compute the ELPD using the PSIS-LOO-CV method, we call the `loo` function. This function's first argument needs to be a DataTree object with a `log-likelihood` group.

```{python}
dt_rugby = az.load_arviz_data('rugby')
elpd = az.loo(dt_rugby, var_name="home_points")
elpd
```
...
Let's see how to use ArviZ to compare two models. We start by simulating some data from a Student's T distribution (df=$\nu$=4), which has heavier tails than a Normal distribution. 
...
We then fit two models, one with a Normal likelihood and another with a Student's T likelihood.
...
stan_data = {"N": len(target), "y": target.tolist()}

# Normal model

## Write the normal model to a file
with open("./normal_model.stan", "w") as f:
    print(normal_model, file=f)

## Compile the model
model_n = CmdStanModel(stan_file="./normal_model.stan")
...
## Convert the fit to an ArviZ DataTree object
idata_n = az.from_cmdstanpy(
    posterior=fit_n,
    log_likelihood="log_lik",
    observed_data={"y": target},
)


# Student's T model

## Write the student model to a file
with open("./student_model.stan", "w") as f:
    print(student_model, file=f)

## Compile the model
model_t = CmdStanModel(stan_file="./student_model.stan")
...
idata_n = az.convert_to_datatree("../models/prerun/model_comparison_00.nc")


if not os.path.exists("../models/prerun/model_comparison_01.nc"):
    idata_t.to_netcdf("../models/prerun/model_comparison_01.nc")
idata_t = az.convert_to_datatree("../models/prerun/model_comparison_01.nc")
```
...
```{python}
cmp_df = az.compare({'model_n':idata_n, 'model_t':idata_t})
cmp_df
```
...
We can obtain similar information, but graphically, using the `az.plot_compare` function
...
### Interpreting ELPD differences

To be able to compare models, we need to understand whether the differences in ELPD are practically meaningful. As a general rule, if the ELPD difference (`elpd_diff`) is less than 4, the difference is considered small and the models have very similar predictive performance [@sivula_2020]. When `elpd_diff` is larger than 4, we should compare it to the standard error of the difference (`dse`). If the difference is several times larger than its standard error, we can be more confident that the models have genuinely different predictive performance.

It is important to remember that even when one model has a higher ELPD, the models may still make very similar predictions for most observations. The ELPD measures the overall quality of the entire predictive distribution, including the tails, and small differences may not be practically relevant.
...
As a general rule, if $\hat{k}$ is less than 0.7 there are no problems; if it is between 0.7 and 1 it is very likely that we are in trouble; and if it is greater than 1, we are doomed. The cut-off value 0.7 is not really fixed, it is computed as $\min\left(1 - \frac{1}{\log_{10}(S)}, 0.7\right))$, where $S$ is the number of posterior draws. When the number of draws is about 2000 (like in our example), the recommended threshold is very close to 0.7. ArviZ can compute the threshold automatically.

The number of samples not only affect the threshold,it can actually help to reduce the value of $\hat{k}$ and thus remove some of these warnings. This is specially true for values close to the threshold, and when the number of samples is not yet "too large" (given our budget). Otherwise, the number needed will be too large to be practically useful.
...
While the main function of LOO is to compare models, the values ​​of $k$ can be useful even if we only have one model. For example, we could have extra knowledge that tells us why these observations are influential, perhaps there was a problem in data collection and the values ​​are incorrect. Or perhaps the values ​​are correct but from the perspective of our model they are influential.
...
### Comparing different observation models

A common question is whether we can compare models with different likelihoods or response distributions using LOO. The answer depends on the types of models being compared:

* **Different continuous likelihoods:** We can compare models with different continuous observation models (e.g., Normal vs. Student-T) as long as the response variable $y$ is exactly the same. If $y$ has been transformed in one model (e.g., log-transformed), the Jacobian of the transformation must be included to make the comparisons valid.

* **Different discrete likelihoods:** We can compare models with different discrete observation models (e.g., Poisson vs. Negative Binomial), since probabilities are not affected by scaling.

* **Continuous vs. discrete models:** We cannot directly compare a continuous model with a discrete model because log-densities and log-probabilities are not on the same scale. To compare them, we would need to discretize the continuous model into probability intervals. See [this case study](https://arviz-devs.github.io/bayesian-workflow/nabiximols/nabiximols.html) for an example of how to do this.

* **Scaling matters:** For continuous models, the scale of $y$ affects log-densities. Comparing a model for $y$ with a model for $\log(y)$ without accounting for the Jacobian will give meaningless results. Always ensure the response variable is on the same scale when comparing continuous models.


## Model averaging and stacking {#sec-model-averaging}

When comparing multiple models, we often find that no single model is clearly the best. In such cases, instead of selecting one model, we can combine predictions from several models. This approach, known as **model averaging**, accounts for model uncertainty and often leads to better predictive performance than selecting a single model.

One simple way to perform model averaging is to turn the ELPD values into weights. We can do this by exponentiating the ELPD values and normalizing them to sum to one. A model with higher ELPD receives more weight in the combination. 

However, a more robust approach is **Bayesian stacking** [@yao_2018]. Instead of using weights derived from individual model performance, stacking optimizes the all the weights to maximize the ELPD of the combined predictive distribution. This is done by solving a constrained optimization problem where the weights sum to one and are non-negative. Bayesian stacking has been shown to perform better than simple LOO-weights, especially when models are correlated or when there is substantial uncertainty about which model is best. By default `az.compare` computes the weights using stacking.

Model averaging is particularly useful when:

* Several models have similar ELPD values (differences less than 4).
* We want to account for model uncertainty in our predictions.
* The models make different predictions for different observations, and combining them can capture a wider range of patterns.

It is important to note that model averaging does not provide a single "best" model, but rather a weighted combination. This can make interpretation more challenging, but the improved predictive performance often justifies the added complexity.

ArviZ provides a `weight_predictions` function that allows us to combine predictions from multiple models using the computed weights. This function takes a list of 
datatrees with `posterior_predictive` and `observed_data` and their corresponding weights, and returns a combined predictive distribution that reflects the contributions of each model according to its weight. The weights can be obtained for example from the `az.compare` function.
...
```{python}
az.loo_expectations(dt_rugby, kind="quantile", probs=[0.25, 0.75])
```
...
```{python}
az.loo_metrics(dt_rugby, kind="rmse", var_name="home_points")
```

Anoth

```{python}
az.loo_r2(dt_rugby, var_name="home_points")
```
...
az.plot_loo_pit(dt_rugby, var_names="home_points")
```

## When not to use cross-validation (or other model selection methods)

In many cases, there is no need to do model selection at all. The best approach is often to build a rich model that includes all relevant uncertainties, perform model checking, and make adjustments if needed. The followin points are discussed in terms of cross-validation, because that's the method we usually use for model comparison, but they apply to other methods as well. 

* **CV does not directly answer "Is there an effect?"** Using CV to compare a model with an additional term to a model without that term is a form of null hypothesis testing. If CV prefers the simpler model, it does not necessarily mean the effect is exactly zero. It may be that the effect is too small to improve predictions, or that it is correlated with other predictors and does not provide additional information. In such cases, examining the posterior distribution of the relevant coefficient is often more informative.

* **Avoid selection-induced bias with many models.** When comparing a large number of models (e.g., in variable selection), the selection process itself can overfit. The model chosen may appear better than it truly is because we selected it based on its performance in a specific sample. If only a few models are compared, this bias is usually negligible. But with many models, more sophisticated approaches such as projective inference may be needed (See @sec-variable-selection).

* **Simple models with no posterior dependencies.** For simple models where parameters are independent and the model is correctly specified, examining the marginal posterior may be sufficient. Cross-validation introduces additional variability and may not provide substantial benefits.


## Other methods for model comparison

As we have already mentioned there are several methods for model comparison we favor PSIS-LOO-CV, but in this section we briefly discuss some other methods that are sometimes used for model comparison.
...
generated quantities {
  real a_prior = beta_rng(1, 1);
}
"""

stan_data = {"N": len(y), "y": y.tolist()}


with open("./model_00.stan", "w") as f:
    print(model_00, file=f)

model_00 = CmdStanModel(stan_file="./model_00.stan")
...
idata_uni = az.convert_to_datatree("../models/prerun/model_comparison_02.nc")
```
...
with open("./model_01.stan", "w") as f:
    print(model_01, file=f)

## Compile the model
model_01 = CmdStanModel(stan_file="./model_01.stan")
...
idata_conc = az.convert_to_datatree("../models/prerun/model_comparison_03.nc")
```
