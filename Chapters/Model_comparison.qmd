...
In this chapter, we provided a theoretical, yet accesible, foundation for a series of method for efficient model comparison. Later in @sec-model-comparison-large-data,@sec-model-comparison-case-study, and @sec-moment-matching we provided applied examples of these methods.

## The balance between simplicity, accuracy
...
This result is very general and useful but it has a catch. In practice we don't really have $\mathbb{E}[\log{q}]$. And hence we need to estimate with. We can use the observed data to estimate the parameters of a model and then use those parameters to estimate $\mathbb{E}[\log{q}]$, but that will introduce a bias. We will be overconfident on the ability of our model to explain the data. Information criteria and cross-validation provides a way to reduce this bias without having to get a new dataset.
...
We have seen that we can use Information criteria estimate the expected log-predictive density (ELPD) by adding a introducing a penalization term to the log-likelihood term, which corrects for the bias introduced when *double-dipping* our model into the data
...
The weights computed in this way are are not normalized, so to use them we need to divide each weight by the total sum of the weights. Once the weights has been normalize we can use them to estimate the ELPD as:
...
The catch, because there is always a catch! Is that the expected $p(\theta^j \mid y_{-i} )$ will in general be "wider" than $p(\theta^j \mid y)$, since it is a posterior distribution estimated with one less observation. This is the opposite to the ideal case in importance sampling. For many cases the difference will be small enough that it will not interfere with our goal. But in some cases the difference will be large enough to render the approximation, unreliable. When? Well, the more "influential" the observation the larger then difference when we remove it. 
...
Theory indicates that under very general conditions the tail of the distribution of weights can be approximated by a Generalized-Pareto distribution. So instead of truncating them we can fit them the Generalized-Pareto distribution to the tail of the computed weights and then replace the computed weight from weights from this fit. This is a form of *smoothing* that, within a certain range, allows stabilizing the importance sampling estimate by making some "very large" weights ​​not so large.
...
In order to be able to compute After all this introduction, calculating LOO may seem somewhat disappointing.

To compare two or more models we can use the `compare` functions that accepts a dictionary with the keys being the names of the models and the values DataTree objects.
...
PSIS-LOO_CV and WAIC converge asymptotically, and they based on the same set of assumptions. So theoretically they are equivalent. However, in practice PSIS-LOO_CV is more robust, and also offers us a diagnosis that indicates when it could be failing (this is thanks to the Pareto adjustment). For this reason ArviZ only implementes PSIS-LOO-CV. We now briefly discuss two other information criteria that are somewhat popular.
...
Bayesian Information Criteria, BIC. It was proposed as a way to correct some of the problems with AIC and the author proposed a Bayesian rationale for it. But BIC is not really Bayesian in the sense that, like AIC, assumes *flat* priors and uses maximum likelihood estimation. But more importantly, BIC differs from AIC and WAIC in its objective. AIC and WAIC try to reflect which model generalizes better to other data (predictive accuracy) while BIC tries to identify which is the _correct_ model and therefore is more related to Bayes factors than with WAIC. In the next section we discuss Bayes Factors and how they differs from PSIS-LOO-CV and WAIC.
