...
In the example for this chapter using the `wells` data, we demonstrate the complete workflow for model comparison with subsampled PSIS-LOO. We fit a logistic regression model using both exact MCMC and Laplace approximations to show how posterior approximations can further reduce computational costs. We implement a lightweight log-likelihood helper that enables efficient evaluation on arbitrary observation subsets, demonstrate subsampled PSIS-LOO using different surrogates (PLPD and LPD), and compare multiple models with subsampled LOO.

## Background

The content contained in this chapter builds on foundational concepts covered in @sec-model-comparison. For detailed explanations of expected log predictive density (ELPD), leave-one-out cross-validation (LOO-CV), Pareto smoothed importance sampling (PSIS), the Watanabe-Akaike information criterion (WAIC), and model comparison principles, please refer to that chapter. Here we focus specifically on the subsampling techniques and difference estimator needed for large datasets.

### From LOO to subsampled LOO

Recall from @sec-model-comparison that PSIS-LOO uses importance sampling to approximate LOO-CV without refitting the model $n$ times. The Pareto-$k$ diagnostic flags unreliable importance sampling approximations when $k > 0.7$. For large $n$, even this efficient computation becomes expensive.
...
2. **Exact PSIS-LOO** computed only on a small random subsample $\mathcal{S}$ of size $m \ll n$
...
The practical appeal of the difference estimator raises natural questions about its theoretical foundation. Under what conditions does it produce valid estimates? How does performance improve as data size increases? 

The key results from @magnusson_2020 show that the estimator converges to the true LOO value under mild regularity conditions. Remarkably, convergence accelerates as data size grows, even with fixed subsample size and fixed number of posterior draws. This means the method becomes more efficient precisely when we need it most.

::: {.callout-note}
## Required assumptions

The theoretical results rely on standard regularity conditions that are satisfied by most common models:

1. **Lipschitz continuity**: The likelihood $p(y|\theta)$ is locally Lipschitz in $\theta$, ensuring smooth posterior behavior
2. **Positive likelihood and posterior**: $p(y|\theta) > 0$ for all observations and parameters in the support
3. **Bounded likelihood**: The likelihood is bounded above to prevent pathological cases
4. **Finite entropy**: The expected negative log-likelihood is finite
5. **Bernstein-von Mises conditions**: Standard asymptotic normality conditions for the posterior
6. **Consistent approximation**: If using approximate inference (variational, Laplace), the approximation $q(\theta|y)$ must be consistent
:::

These conditions are mild and hold for regular parametric models including regression, GLMs, and hierarchical models.

### Workflow for model comparison

The workflow for applying subsampled PSIS-LOO to model comparison is straightforward. We compute cheap surrogates for all observations, then refine our estimates using PSIS-LOO on a small subsample, balancing computational efficiency with statistical accuracy:
...
4. Calculate $\pi_j$ for the subsample using PSIS-LOO, checking Pareto-$k$ diagnostics for unreliable importance sampling approximations
...
We can fit the model with PyMC where posterior draws are stored in a `DataTree` object.
...
## Subsampled PSIS-LOO with the PLPD surrogate

As we've discussed previously, a key component of the PSIS-LOO subsampling method is the notion of a fast surrogate for the leave-one-out term $\pi_i$. One such surrogate is the point log predictive density (PLPD) which @magnusson_2020 motivate by replacing the leave-one-out term $\pi_i$ with
...
## Subsampled PSIS-LOO with the LPD surrogate
...
```{python}
from arviz_stats import update_subsample

loo_update = update_subsample(
    loo_plpd,
    data,
    observations=200,
    var_name="y",
...
loo_ap = loo_approximate_posterior(
    data=data_laplace,
    log_p=log_p_da,
    log_q=log_q_da,
    var_name="y",
...
The following computes full PSIS-LOO for the first model using the `loo()` function.

```{python}
from arviz_stats import loo

loo_full = loo(data, var_name="y", pointwise=True)
loo_full
```
