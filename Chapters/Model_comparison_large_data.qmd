...
We can fit the model using PyMC, Bambi, or CmdStanPy where posterior draws are stored in a `DataTree` object.
...
Here we use the `method="plpd"` argument to `loo_subsample()` to use the PLPD surrogate. Computing the surrogate requires the log-likelihood of each observation, so we pass our fitted model through the `model` argument and ArviZ compiles the log-likelihood function it needs directly from the model.
...
If we are working with a framework that does not support automatic extraction, or we want full control over the computations, we can write the log-likelihood function ourselves. `loo_subsample()` accepts a callable with signature `log_lik_fn(observed, data)` so that the log-likelihood can be computed for whichever subset of observations it selects on demand, rather than stored for all observations, which can be expensive for large datasets. We can always wrap auxiliary parameters with tools such as `functools.partial` so the routine still receives `(observed, data)`.
...
This log-likelihood helper evaluates the Bernoulli log-pmf in its logit form, which is numerically stable for extreme probabilities, and returns log probabilities for each posterior draw. No log-likelihood values are stored permanently, which keeps memory usage modest even when $N$ is large. When passing `log_lik_fn`, we also need `param_names` to tell `loo_subsample()` which posterior variables form the point estimate for the PLPD surrogate.
...
The estimates are identical to the ones we got by passing the model directly, as they should be. ArviZ builds the equivalent of this function from the model behind the scenes, so both routes run the same computation on the same subsample.
