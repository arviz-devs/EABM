...
This chapter only briefly touches on the Bayesian workflow; many of its elements were covered earlier, but not tied together into a single narrative, which is central to the notion of "workflow". Readers looking to explore this topic further may want to turn to @gelman_2026, which covers it in much more depth. The book includes a series of case studies using tools from the R ecosystem. These same case studies have been ported to ArviZ and other tools in the Python ecosystem, and are available [here](https://arviz-devs.github.io/EABM/).
