...
As we can see, the roaches dataset exhibits substantial variation in roach counts with considerable overdispersion (mean 25.6, standard deviation 50.8). Approximately 36% of apartments had zero roaches caught during the follow-up period, while the maximum count reached 357. The exposure period varies from 0.2 to 4.3 days across apartments, necessitating the use of an offset term in the model. This combination of high variability, many zeros, and extreme counts makes certain observations potentially influential, which is precisely the scenario where moment matching becomes valuable for improving LOO-CV estimates.
...
Now we apply importance weighted moment matching to improve the PSIS-LOO-CV estimates for the problematic observations. In ArviZ, this is done using the `loo_moment_match()` function, which requires us to provide functions that can evaluate the model's log probability and log likelihood in the unconstrained parameter space.

### Function specification and implementation

For PSIS-LOO-CV moment matching to work, we need to provide:

- **`log_prob_upars_fn`**: A function that computes the log posterior density $\log p(\theta \mid y)$ for unconstrained parameters
