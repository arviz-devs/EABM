...
$$
w_{\text{loo}, i}^{*(s)} = \frac{p(\theta^{*(s)} \mid y)}{p(\theta^{(s)} \mid y) \, p(y_i \mid \theta^{*(s)})},
$$

which is still proportional to the ratio of the LOO posterior to the original proposal $p(\theta^{(s)} \mid y_{-i}) / p(\theta^{(s)} \mid y)$. This reweighting accounts for both the density of the transformed draw under the full-data posterior and its likelihood for observation $i$. The denominator term $p(\theta^{(s)} \mid y)$ remains constant for each draw across all LOO folds, but the numerator terms must be evaluated at each transformed location.
...
Having defined the three transformations, we can now see how they work together in practice. The algorithm starts with the simplest transformation (T1) and only proceeds to more complex ones when necessary, using the Pareto-$k$ diagnostic to decide whether each transformation improves the approximation. It implements the double adaptation strategy by running two sequential loops: one targeting the numerator using absolute expectation-specific weights, and another targeting the denominator using common weights. Both adapted proposals are then combined via multiple importance sampling.
...
Because the numerator and denominator adaptations are combined via MIS, @paananen_2020 split the original Monte Carlo sample into two halves so the two adapted proposals use disjoint draws, avoiding bias from reusing the same draws in both components. ArviZ automates this entire workflow through the `loo_moment_match()` function.
