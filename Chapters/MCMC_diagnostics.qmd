...
sample = az.convert_to_dataset({"good_sample":good_sample,
...
In ArviZ by calling the function `az.plot_trace_dist(.)` we get a trace plot on the right and on the left a KDE @fig-tracedistplot_good_sample is an example of this. 


```{python}
#| label: fig-tracedistplot_good_sample
#| fig-cap: "Trace plot and Kernel Density Estimate (KDE) of a sample without issues"
az.plot_trace_dist(sample, var_names="good_sample", compact=False);
```

As discussed in @sec-dist, there are many methods to represent distributions. We can use the argument `kind` to define other methods:
...
Alternatively, if we just want the trace without the density representation we can use the function `az.plot_trace(.)`
...
#| code-fold: true
#| fig-cap: "Trace plot of 4 chains that has not converged"
az.plot_trace_dist(sample, var_names="bad_sample_0", compact=False);
```
...
pc = az.plot_trace_dist(sample, var_names="bad_sample_1", compact=False)
pc.coords = {"column": "trace"}
az.add_bands(pc, stuck)
```
...
```{python}
#| label: fig-rank_plot_good
#| code-fold: true
#| fig-cap: "Rank plot for `good_sample`"
az.plot_rank(sample, var_names=["good_sample"]);
```

For `good_sample` we can see that ranks looks relative flat and deviations are within the envelope. In sharp contrast `bad_sample_0` looks awful, deviations are very large as chains belong to different distributions. The envelopes computation does not take into account autocorrelation, so it is recommend to thin the draws as discussed in @sec-thinning. `plot_rank` does this automatically. To disable thinning we can do `az.plot_rank(., thin=False)`, as we did for @fig-rank_plot_bad_sample_0. 

```{python}
#| label: fig-rank_plot_bad_sample_0
#| code-fold: true
#| fig-cap: "Rank plot for `bad_sample_0`"
az.plot_rank(sample, var_names=["bad_sample_0"], thin=False);
```

Finally, `bad_sample_1` also shows problems, see how chain 0 (blue) get out of the envelope. Notice also how the envelope and lines looks very "squared-shaped" compared with those from @fig-rank_plot_good. This is indicating a relatively low ESS (high thinning). This is a consequence of the chain 0 (blue) being stuck in some regions as shown in @fig-traceplot_bad_sample_1.


```{python}
#| label: fig-rank_plot_bad_sample_1
#| code-fold: true
#| fig-cap: "Rank plot for `bad_sample_1`"
az.plot_rank(sample, var_names=["bad_sample_1"]);
```
...
Using ArviZ we can get the $\hat R$ with `az.rhat(⋅)`, `az.summary(⋅)` and `az.plot_forest(⋅, r_hat=True)`
...
Another plot that can be useful to diagnose divergences is `az.plot_parallel()`, which shows the samples in parallel coordinates. A parallel coordinates plot is a way to visualize high-dimensional data by plotting each dimension as a vertical line and connecting the points across dimensions with lines. 

From @fig-parallel_plot we can see that the `centered` model has divergences in the region where `tau` takes small values.

```{python}
#| label: fig-parallel_plot
#| fig-cap: "Parallel coordinates plot for `centereed` model."
az.plot_parallel(
    idata_cm,
    var_names=['theta', 'tau'],
    figure_kwargs={"figsize": (9, 4)},
    visuals={"xticks":{"rotation": 45}}
    )
```
