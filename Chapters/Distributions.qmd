...
#| fig-cap: "A gamma distribution with dashed lines representing its mean, median, and mode."
dist = pz.Gamma(3, 2)
mean = dist.mean()
median = dist.median()
mode = dist.mode()
...
```{python}
pois_data = dist.rvs(50_000, random_state=rng)
az.mean(pois_data), az.median(pois_data), az.mode(pois_data)
```
...
```{python}
az.rcParams["stats.point_estimate"]
```
...
```{python}
az.hdi(pois_data), az.eti(pois_data)
``` 
...
d_values = az.convert_to_dataset(pz.Poisson(3).rvs((1, 500), random_state=rng))
az.plot_dist(d_values, kind="hist");
```
...
d_values = az.convert_to_dataset(pz.Poisson(100).rvs((1, 500), random_state=rng))
az.plot_dist(d_values, kind="hist");
```
...
d_values = az.convert_to_dataset(pz.Poisson(100).rvs((1, 500), random_state=rng))
az.plot_dist(d_values, kind="hist", stats={"dist":{"bins":20}})
```
...
#| fig-cap: "KDE from a sample of floats. By default, ArviZ computes a KDE."
c_values = az.convert_to_dataset(pz.Gamma(2, 3).rvs((1,1000), random_state=rng))
...
az.plot_dist(c_values, kind="ecdf");
```

### Quantile dot plots

A quantile dot plot [@wilkinson_1999; @kay_2016] displays the distribution of a sample in terms of its quantiles. Reading the median or other quantiles from quantile dot plots is generally easy, we just need to count the number of dots.
...
d_values = az.convert_to_dataset(pz.Normal(2, 2).rvs((4, 1000), random_state=123))
az.plot_dist(
    d_values,
    kind="dot",
...
### Evaluating the fit of a distribution

A simple way to evaluate the fit of a distribution is to plot the estimated density and inspect it visually. For instance, if we know that the data is positive and skewed, we can check if the estimated density has those features. If the estimated density has negative values or if it is symmetric, then we know that the fitted distribution is not a good model for the data. If we are using a KDE unadvertely on discrete data with few unique values, we will get a series of peaks and valleys instead of a smooth curve, which is a clear sign that the KDE is not a good model for the data. Of course this sanity check is useful to detect gross misfits, but it is not very useful to detect more subtle misfits. For that reason we may want to use a more quantitative method to evaluate the fit of a distribution, like the `Δ-ECDF-PIT` diagnostic that we will discuss in @sec-mcmc-diagnostics [@Sailynoja_2025]. This method is implemented in ArviZ via two functions `plot_dgof()` (just the diagnostics) and `plot_dgof_dist()` (diagnostics plus the estimated density).

Figure @fig-dgof_dist_ok shows a good fit of a KDE to the data, while @fig-dgof_dist_bad shows a poor fit, the low p-value and the highlighted points (red) in the right plot of @fig-dgof_dist_bad indicates that the KDE has some issues fitting the data.


```{python}
#| label: fig-dgof_dist_ok
#| fig-cap: "Δ-ECDF-PIT diagnostic for a KDE. The left plot shows the estimated density and the right plot shows the Δ-ECDF-PIT diagnostic. Deviations from the uniform distribution in the right plot indicate a poor fit of the KDE to the data."
az.plot_dgof_dist(d_values, kind="kde");
```


```{python}
#| label: fig-dgof_dist_bad
#| fig-cap: "Δ-ECDF-PIT diagnostic for a KDE. The left plot shows the estimated density and the right plot shows the Δ-ECDF-PIT diagnostic. Deviations from the uniform distribution in the right plot indicate a poor fit of the KDE to the data."
e_values = az.convert_to_dataset(pz.Exponential(1).rvs((1, 1000), random_state=235))
az.plot_dgof_dist(e_values, kind="kde")
```

In @sec-mcmc-diagnostics, we discuss details on how the `Δ-ECDF-PIT` plot are computed and how to interpret them. 