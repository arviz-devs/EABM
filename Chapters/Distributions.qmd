...
@fig-point-estimates shows an example of the mode, mean, and median of a `Gamma` distribution.
...
for idx, (stat, value) in enumerate(zip(
    [f"mean    = {mean:.2f}", f"median = {median:.2f}", f"mode    = {mode:.2f}"], [mean, median, mode]
)):
...
azp.mean(pois_data), azp.median(pois_data), azp.mode(pois_data)
```
...
For asymmetric distribution, the HDI is usually preferred over the ETI because it better represents the most credible values of the distribution.
...
dist = pz.Gamma(3, 2)
ax[0] = dist.plot_pdf(ax=ax[0], legend="title", pointinterval=True, interval="hdi")
ax[1] = dist.plot_pdf(ax=ax[1], legend=None, pointinterval=True, interval="eti")
```
