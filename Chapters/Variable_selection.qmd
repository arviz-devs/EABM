...
with pm.Model() as bart_model:
    ...
    pm.NegativeBinomial("count", mu=μ, alpha=α, observed=Y)
...
import os
import arviz as az
bambi_model0 = bmb.Model("count ~" +  " + ".join([c for c in bikes.columns if c!="count"]), data=bikes, family="negativebinomial")
bambi_model0.build()
...
```{python}
ppi = kpt.ProjectionPredictive(bambi_model0, idata)
ppi.project()
```
...
```{python}
#| eval: false
bambi_model1 = bmb.Model("count ~" +  " + ".join([c for c in bikes.columns if c!="count"]), data=bikes, family="negativebinomial")
idata = bambi_model1.fit(idata_kwargs={'log_likelihood': True}, random_seed=seed)
```
...
```{python}
ppi = kpt.ProjectionPredictive(bambi_model1, idata)
ppi.project()
kpt.plot_compare(ppi.compare(min_model_size=1));
```
...
Both methods are not necessarily competitors and there is room for interaction. For instance, we could use PyMC-BART to find the most relevant variables, according to the variable inclusion metric, and then use Kulprit to perform the projection and evaluation of those selected submodels. 

```{python}
terms = pmb.vi_to_kulprit(vi_results)
ppi.project(user_terms=terms)
```
In this case, we are using the `vi_to_kulprit` function to convert the variable importance results from PyMC-BART into a list of submodels that Kulprit can use. And then we call the `project` method with the `user_terms` argument to specify the submodels we want to project and evaluate. Notice that if we provide the `user_terms` there is no search done, Kulprit will project and evaluate only the specified submodels.

Of course, we can also specify the submodels manually. For instance, we could use the following list of submodels:
...
Another option is to use the results from the BART model as the reference model for Kulprit. One restriction is that the name used for the response variable in the BART model must be the same as the one used in the Bambi model.
```{python}
#| eval: false
with bart_model:
    pm.sample_posterior_predictive(idata_bart, extend_inferencedata=True)

ppi = kpt.ProjectionPredictive(model, idata_bart)
ppi.project()
```