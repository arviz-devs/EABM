...
# Prior and likelihood sensitivity checks {#sec-sensitivity-checks}
...
dt_bf_00 = az.convert_to_datatree("../models/prerun/sensitivity_00.nc")
```
...
```{python}
az.psense_summary(dt_bf_00, var_names=["~μ"], round_to=2)
```
...
az.plot_psense_quantities(dt_bf_00,
                           var_names=["β"],
                           coords={"covariates": ["wrist"]})
```
...
dt_bf_01 = az.convert_to_datatree("../models/prerun/sensitivity_01.nc")
```
...
```{python}
az.psense_summary(dt_bf_01, var_names=["~μ"])
```
...
az.plot_psense_quantities(dt_bf_01, var_names=["β"], coords={"covariates": ["wrist"]})
```
...
```{python}
r2_da = az.ndarray_to_dataarray(az.bayesian_r2(dt_bf_01, pred_mean="μ", scale="σ", summary=False).reshape(4, 2000), var_name="r2")
...
```{python}
az.psense_summary(dt_bf_01, var_names=["r2", "log_score"])
```
...
dt_bt = az.convert_to_datatree("../models/prerun/sensitivity_02.nc")
```
...
```{python}
az.psense_summary(dt_bt, var_names=["~b_Intercept"])
```
